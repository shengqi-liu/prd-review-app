...
> 项目骨架能力，约定 Maven 模块结构、Spring Boot 运行时、配置约定、统一异常与响应、日志与 traceId、OpenAPI 文档与健康检查等基础设施级要求。
...
项目 SHALL 使用 Logback 输出 JSON 结构化日志，并通过 Servlet Filter 在请求入口处生成 `traceId`（UUID 去横线）写入 MDC，响应返回时通过响应头 `X-Trace-Id` 暴露。所有业务日志 MUST 自动携带 `traceId` 字段。客户端传入的 `traceId` MUST 经过换行符过滤与长度截断（≤64字符）后方可写入 MDC，防止 Log Injection。
...
#### Scenario: Log Injection 防护
- **WHEN** 客户端请求头 `X-Trace-Id` 中包含换行符
- **THEN** 系统 MUST 过滤换行符后再写入 MDC，不得将原始值直接写入日志
