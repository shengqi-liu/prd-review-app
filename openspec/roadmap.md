...
| 4.5 | `add-ai-infrastructure` | Spring AI 抽象 + SSE + DocumentFetcher | #4 | ✅ DONE | 2026-06-14 |

## 阶段二 · 基础领域（可并行）

| # | Change ID | 名称 | 依赖 | 状态 | 完成日期 |
|---|-----------|------|------|------|---------|
| 5 | `add-prd-storage` | PRD 实体 + CRUD + 版本表（MyBatis-Plus） | #4 | ✅ DONE | 2026-06-09 |
| 5.1 | `add-prd-frontend` | 前端 PRD 列表/编辑器接通后端 CRUD + submit | #5 | ✅ DONE | 2026-06-21 |
| 5.2 | `add-prd-review-flow-frontend` | 评审流程页接通后端 + 评审详情 + URL hash 路由 + 锁定 + 进度记录 | #5.1 | ✅ DONE | 2026-06-22 |
| 6 | `add-prd-input-validation` | 最低输入门槛 + 章节解析 | #5 | ✅ DONE | 2026-06-21 |
| 7 | `add-document-parsing` | PDF/Word → 结构化文本（Tika） | #5 | ✅ DONE | 2026-06-21 |
| 8 | `add-reviewer-management` | AI 评审员 CRUD + Prompt 模板 | #4 | ✅ DONE | 2026-06-14 |
| 9 | `add-reviewer-test-endpoint` | Prompt 试跑接口（SSE 流式） | #8 | ✅ DONE | 2026-06-15 |
| 10 | `add-review-styles` | 评审风格 + 规则配置 | #4 | ✅ DONE | 2026-06-19 |

## 阶段三 · 知识库与 RAG（独立链路）

| # | Change ID | 名称 | 依赖 | 状态 | 完成日期 |
|---|-----------|------|------|------|---------|
| 11 | `add-kb-git-watcher` | JGit 轮询 + 变更检测 | #1 | ✅ DONE | 2026-06-20 |
| 11.1 | `fix-kb-sync-stuck-recovery` | JGit timeout + 启动清理（修复 #11 SYNCING 卡死） | #11 | ✅ DONE | 2026-06-21 |
| 11.2 | `fix-kb-sync-correctness` | 修复 #11 乐观锁同步 + 事务时序两个 bug | #11 | ✅ DONE | 2026-06-21 |
| 12 | `add-kb-markdown-indexing` | Markdown 分块 + BGE Embedding + Chroma 写入 | #11 | 🔲 NOT_STARTED | — |

> **⚠ #12 设计阶段必决项 — 事件投递可靠性（来自 2026-06-21 explore）**
>
> #11 的 `KbDocumentChangedEvent` 当前用 `ApplicationEventPublisher` 同步发布。#12 作为消费者必须明确选择监听策略，并在 `design.md` 写清理由：
>
> - **A. 同步 `@EventListener`**：失败回滚 → 重启自动重发；但监听器慢/失败会拖死整批 git 同步
> - **B. `@TransactionalEventListener(AFTER_COMMIT)` + `@Async` + 重试**：解耦同步任务；JVM crash 在投递期间会**永久丢失**
> - **C. Outbox 模式**：新增 `kb_pending_event` 表，与 git 同步同事务持久化；独立 worker 消费 + ack。最可靠但工程量 +30%
>
> 推荐路径：MVP 增量场景用 B；**首次 clone 全量 ADDED**（几百份文档一次性投递）必须走 A 或 C，否则崩了无法补救（diff 不会再把"已存在"文件标 ADDED）。
>
> 风险评估见归档：`openspec/changes/archive/2026-06-20-add-kb-git-watcher/design.md` D6 段；以及 #11 之后的 SYNCING 看门狗 change。
...
| 16 | `add-llm-response-cache` | LLM 非流式响应精确 hash 缓存(降本提速) | #4.5 | ✅ DONE | 2026-06-28 |
