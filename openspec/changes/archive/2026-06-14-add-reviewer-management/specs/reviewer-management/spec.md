...
- ADMIN 用户 SHALL 看到"+ 新建评审员"、"✏️ 编辑 Prompt"、"启用/停用"、"🗑️ 删除"按钮；非 ADMIN 用户 SHALL 仅看到只读卡片
...
- "🧪 测试"按钮 SHALL 显示为 disabled 状态（待 change #9 实现）
...
- **THEN** 页面 MUST 仅显示 enabled=true 的 4 位评审员，且不显示编辑/删除按钮
